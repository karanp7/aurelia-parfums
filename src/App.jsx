...
function App() {
  ...
  const crossSell = useMemo(() => {
    ...
    if (discovery) return { type: 'bottle', product: perfumes.find((product) => discovery.matches?.includes(product.name)) || perfumes[1] };
    if (cart.some((item) => item.type === 'bottle')) return { type: 'discovery', product: discoverySet };
  ...
  return <div className="site-shell">
    ...
    <main id="top">
      ...
      <section id="collection" className="collection" data-reveal>
        ...
        <div className="finder-row">
          ...
          <div className="family-tabs">{families.map((item) => <button key={item} className={family === item ? 'active' : ''} onClick={() => setFamily(item)}>{item}</button>)}</div>
        </div>
    ...
    {activeProduct && <div className="overlay modal-layer" role="dialog" aria-modal="true" aria-label={`${activeProduct.name} details`}>
      <div className="product-modal">
        ...
        <div className="modal-copy"><p className="overline dark">{activeProduct.house}</p><h2>{activeProduct.name}</h2><div className="rating">★★★★★ <span>{activeProduct.rating} from {activeProduct.reviews} reviews</span></div><p className="plain-description">{activeProduct.description}</p>
          ...
          <fieldset className="size-selector"><legend>Choose size</legend>{activeProduct.sizes.map((size) => <button key={size.label} className={selectedSize === size.label ? 'selected' : ''} onClick={() => setSelectedSize(size.label)}><span>{size.label}</span><strong>{money(size.price)}</strong></button>)}</fieldset>
    ...
    {quizOpen && <div className="overlay quiz-layer" role="dialog" aria-modal="true" aria-label="Scent finder">
      <div className="quiz-modal"><button className="close" onClick={closeQuiz} aria-label="Close quiz">×</button><div className="quiz-progress"><span style={{ width: `${Math.min(100, ((quizStep + 1) / 5) * 100)}%` }}/></div>
        ...
        {quizStep === 2 && <QuizQuestion eyebrow="The feeling" title="How should the fragrance make them feel?" options={['Elegant', 'Confident', 'Energizing', 'Comforting']} onAnswer={answerQuiz}/>} 
        ...
        {quizStep >= 4 && <div className="quiz-result"><p className="overline dark">Your three-scent edit</p><h2>{quizAnswers[0] === 'Gift' ? 'A thoughtful shortlist.' : 'Try these on your skin.'}</h2><p>Varied enough to learn from, connected enough to feel personal.</p><div className="result-grid">{quizMatches.map((product, index) => <article key={product.id}><span>{index === 0 ? 'Strongest match' : index === 1 ? 'Softer alternative' : 'More adventurous'}</span><div className={`mini-art tone-bg-${product.tone}`}><PerfumeBottle tone={product.tone} compact image={product.image} alt={product.imageAlt || product.name}/></div><h3>{product.name}</h3><p>{product.summary}</p></article>)}</div><button className="btn btn-dark full" onClick={() => { addDiscoverySet(quizMatches); closeQuiz(); }}>Try all three — $18</button><button className="retake" onClick={() => {setQuizStep(0);setQuizAnswers({});}}>Retake quiz</button></div>}
      </div>
    </div>}

    {cartOpen && <div className="overlay align-right" role="dialog" aria-modal="true" aria-label="Shopping bag"><div className="cart-drawer"><div className="drawer-head"><h2>Your bag</h2><button className="close static" onClick={() => setCartOpen(false)}>×</button></div>
      {!cart.length ? <div className="empty"><h3>Nothing chosen yet.</h3><p>Start with a personalized discovery set if you are still learning what you love.</p><button className="btn btn-dark" onClick={() => {setCartOpen(false);setQuizOpen(true);}}>Find my scents</button></div> : <>
        ...
        <div className="cart-list">{cart.map((item) => <article key={item.key}><div className="cart-art">{item.type === 'discovery' ? <div className="mini-set">3</div> : <PerfumeBottle tone={item.tone} compact image={item.image} alt={item.name}/>}</div><div><small>{item.type === 'discovery' ? 'Discovery set' : item.house}</small><h3>{item.name}</h3><p>{item.size}</p>{item.matches && <p>{item.matches.join(' · ')}</p>}<div className="quantity"><button onClick={() => updateQuantity(item.key, -1)}>−</button><span>{item.quantity}</span><button onClick={() => updateQuantity(item.key, 1)}>＋</button></div></div><strong>{money(item.price * item.quantity)}</strong></article>)}</div>
        <div className="cart-options"><label><input type="checkbox"/> Add premium gift wrap — $8</label><label htmlFor="gift-message">Gift message</label><textarea id="gift-message" placeholder="Optional message for the recipient"/></div>
